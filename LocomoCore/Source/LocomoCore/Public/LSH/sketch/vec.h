...
#include <array>
#ifndef NO_BLAZE
#include "BLAZESubset/IsolatedBlazeIncludes.h"
#endif
#if defined(_MSC_VER) && !defined(__clang__)
// Bitwise AND operator for __m512i
inline __m512i operator&(__m512i a, const __m512i& b)
{
...
#define decop(op, suf, sz) template<typename... Args> static INLINE auto op(Args&&... args) { return OP(op, suf, sz)(std::forward<Args>(args)...);}
#define specialize(name, op, suf, sz) template<typename... Args> static INLINE auto name(Args&&... args) { return OP(op, suf, sz)(std::forward<Args>(args)...);}
#define decopO(op, suf, sz) template<typename... Args> INLINE auto operator##op(Args&&... args) { return OP(op, suf, sz)(std::forward<Args>(args)...);}
...
#define declare_avx512_cmpeq_mask(sz) \
    template<typename... Args> static INLINE auto cmpeq_mask(Args&&... args) { return _mm512_cmpeq_epi##sz##_mask(std::forward<Args>(args)...);} \
    template<typename... Args> static INLINE auto cmpneq_mask(Args&&... args) { return _mm512_cmpneq_epi##sz##_mask(std::forward<Args>(args)...);}
...
#define declare_int_epi32_128(sz) \
    ...
    decop(min, epu32, sz) \
    decop(max, epu32, sz) \
    template<typename... Args> static INLINE auto mul(Args&&... args) { return _mm_mullo_epi32(std::forward<Args>(args)...);} \
    template<typename... Args> static INLINE auto mullo(Args&&... args) { return _mm_mullo_epi32(std::forward<Args>(args)...);} \
    decopO(xor, si128, sz) \
    decopO(and, si128, sz) \
    decopO(or, si128, sz) \
    decop(set1, epi32, sz) \
...
#define declare_int_epi16(sz) \
    ...
    decopO(xor, si##sz, sz)\
    template<typename... Args> static INLINE auto mul(Args&&... args) { return OP(mullo, epi16, sz)(std::forward<Args>(args)...);}\
    decopO(or, si##sz, sz)\
    decopO(and, si##sz, sz)\
    decop(set1, epi16, sz) \
...
#define declare_int_epi16_128(sz) \
    ...
    decop(set1, epi16, sz) \
    decop(cmpeq, epi16, sz) \
    template<typename... Args> static INLINE auto mul(Args&&... args){ _mm_mullo_epi16(std::forward<Args>(args)...);} \
    template<typename... Args> static INLINE auto mullo(Args&&... args){ _mm_mullo_epi16(std::forward<Args>(args)...);}
...
#define declare_int_epi64(sz) \
    ...
    decopO(or, si##sz, sz) \
    decopO(and, si##sz, sz) \
    specialize(and_fn, and, si##sz, sz) \
    decop(set1, epi64x, sz) \
    decop(cmpeq, epi64, sz) \
    template<typename... Args> static INLINE auto mul(Args&&... args){ OP(mullo, epi64, sz)(std::forward<Args>(args)...);}
...
#define declare_int_epi64_128(sz) \
    ...
    decop(set1, epi64x, sz) \
    decop(cmpeq, epi64, sz) \
    template<typename... Args> static INLINE auto mul(Args&&... args){ _mm_mullo_epi64(std::forward<Args>(args)...);} \
    template<typename... Args> static INLINE auto mullo(Args&&... args){ _mm_mullo_epi64(std::forward<Args>(args)...);}
   ...
   dec_sleefop_noprec(trunc, suf, set)
	...
	union UType
	{
		...
		struct unroller<0, done>
		{
			...
			template <typename Functor>
			constexpr void for_each(const Functor&)
			{
			}
	...
	template <>
	struct SIMDTypes<int64_t> : public SIMDTypes<uint64_t>
	{
	};
...
#error("Need at least sse2")
	...
	template <>
	struct SIMDTypes<int32_t> : public SIMDTypes<uint32_t>
	{
	};
...
#error("Need at least sse2")
	...
	template <>
	struct SIMDTypes<int16_t> : public SIMDTypes<uint16_t>
	{
	};
...
#error("Need at least sse2")
	...
	template <>
	struct SIMDTypes<int8_t> : public SIMDTypes<uint8_t>
	{
	};
...
#define DO_DUFF(len, ITER) \
	...
	void vecmul(FloatType* to, const FloatType* from, size_t nelem)
	{
		...
		if (!(Space::aligned(ptr) && Space::aligned(fromptr)))
			while (reinterpret_cast<FloatType*>(ptr) < end - sizeof(SIMDType) / sizeof(FloatType))
				Space::storeu(reinterpret_cast<FloatType*>(ptr),
				              Space::mul(Space::loadu(reinterpret_cast<const FloatType*>(fromptr)),
				                         Space::loadu(reinterpret_cast<FloatType*>(ptr)))),
					++ptr, ++fromptr;
		else
			while (reinterpret_cast<FloatType*>(ptr) < end - sizeof(SIMDType) / sizeof(FloatType))
				Space::store(reinterpret_cast<FloatType*>(ptr),
				             Space::mul(Space::load(reinterpret_cast<const FloatType*>(fromptr)),
				                        Space::load(reinterpret_cast<FloatType*>(ptr)))),
					++ptr, ++fromptr;
